...
* [x] Replace "X" with a reference to `state.operation` within the operation element (like a tag).
...
* [x] 
Note the `APPLY_NUMBER` action case (in ./reducer/index.js) 
and:
`applyNumber` action creator (in ./actions/index.js). This action adds, multiplies or subtracts a number passed into the action creator.
